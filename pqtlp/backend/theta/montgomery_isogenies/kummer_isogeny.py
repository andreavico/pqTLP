...
can be lifted from the KummerLine `phi.codomain().curve()`.
...
class KummerLineIsogeny_Generic:
    ...
    def validate_input(domain, kernel, degree, check=True):
        ...
        if check:
            # Check annihilation; callers must supply a full-order generator.
            assert (degree * kernel).is_zero(), (
                "Input point does not have correct order"
            )
...
class KummerLineIsogeny_Velu(KummerLineIsogeny_Generic):
    ...
    A degree-two kernel at (0,0) requires a different curve model; callers
    must use another isogeny implementation for that case.
    """
