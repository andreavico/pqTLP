...
from ..theta_structures.dimension_two import ThetaPoint, ThetaStructure
...
class ThetaIsogeny2(ThetaIsogeny4):
    ...
    # T1 and T2 preserve the parent interface; this step uses only the domain.
    def _compute_codomain(self, T1, T2):
        """Compute the codomain from the domain theta null point."""
        if self._hadamard[0]:
            AA, BB, CC, DD = ThetaPoint.to_squared_theta(
                *ThetaPoint.to_hadamard(*self._domain.coords())
            )
